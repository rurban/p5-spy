...
#[repr(C)]
#[derive(Copy, Clone)]pub union _xmgu { pub xmg_magic : * mut MAGIC , pub xmg_hash_index : STRLEN , _bindgen_union_align : u64 , }#[test]
...
impl ::std::fmt::Debug for pmop {
    fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
        write!(
            f,
            "pmop {{ op_next: {:?}, op_sibparent: {:?}, op_ppaddr: {:?}, op_targ: {:?}, op_type : {:?}, op_opt : {:?}, op_slabbed : {:?}, op_savefree : {:?}, op_static : {:?}, op_folded : {:?}, op_moresib : {:?}, op_spare : {:?}, op_flags: {:?}, op_private: {:?}, op_first: {:?}, op_last: {:?}, op_pmoffset: {:?}, op_pmflags: {:?}, op_pmreplrootu: {:?}, op_pmstashstartu: {:?}, op_code_list: {:?} }}",
            self.op_next,
            self.op_sibparent,
            self.op_ppaddr,
            self.op_targ,
...
fn bindgen_test_layout_yy_parser() {
    ...
    assert_eq!(
        unsafe { &(*(::std::ptr::null::<yy_parser>())).sig_elems as *const _ as usize },
        616usize,
        concat!(
            "Offset of field: ",
            stringify!(yy_parser),
    ...
    assert_eq!(
        unsafe { &(*(::std::ptr::null::<yy_parser>())).sig_optelems as *const _ as usize },
        624usize,
        concat!(
            "Offset of field: ",
            stringify!(yy_parser),
    ...
    assert_eq!(
        unsafe { &(*(::std::ptr::null::<yy_parser>())).sig_slurpy as *const _ as usize },
        632usize,
        concat!(
            "Offset of field: ",
            stringify!(yy_parser),
            "::",
            stringify!(sig_slurpy)
        )
    );
    assert_eq!(
        unsafe { &(*(::std::ptr::null::<yy_parser>())).recheck_utf8_validity as *const _ as usize },
        633usize,
        concat!(
            "Offset of field: ",
            stringify!(yy_parser),
...
impl ::std::fmt::Debug for yy_parser {
    fn fmt(&self, f: &mut ::std::fmt::Formatter) -> ::std::fmt::Result {
        write!(
            f,
            "yy_parser {{ old_parser: {:?}, yylval: {:?}, yychar: {:?}, yyerrstatus: {:?}, yylen: {:?}, stack: {:?}, stack_max1: {:?}, ps: {:?}, lex_brackets: {:?}, lex_casemods: {:?}, lex_brackstack: {:?}, lex_casestack: {:?}, lex_defer: {:?}, lex_dojoin: {:?}, expect: {:?}, preambled: {:?}, lex_formbrack: {:?}, lex_inpat: {:?}, lex_op: {:?}, lex_repl: {:?}, lex_inwhat: {:?}, last_lop_op: {:?}, lex_starts: {:?}, lex_stuff: {:?}, multi_start: {:?}, multi_end: {:?}, multi_open: {:?}, multi_close: {:?}, lex_re_reparsing: {:?}, lex_super_state: {:?}, lex_sub_inwhat: {:?}, lex_allbrackets: {:?}, lex_sub_op: {:?}, lex_sub_repl: {:?}, lex_shared: {:?}, linestr: {:?}, bufptr: {:?}, oldbufptr: {:?}, oldoldbufptr: {:?}, bufend: {:?}, linestart: {:?}, last_uni: {:?}, last_lop: {:?}, copline: {:?}, in_my: {:?}, lex_state: {:?}, error_count: {:?}, in_my_stash: {:?}, rsfp: {:?}, rsfp_filters: {:?}, nextval: {:?}, nexttype: {:?}, nexttoke: {:?}, form_lex_state: {:?}, lex_fakeeof: {:?}, lex_flags: {:?}, saved_curcop: {:?}, tokenbuf: [{}], herelines: {:?}, preambling: {:?}, sig_elems: {:?}, sig_optelems: {:?}, sig_slurpy: {:?}, recheck_utf8_validity: {:?}, in_pod : {:?}, filtered : {:?}, saw_infix_sigil : {:?}, parsed_sub : {:?}, sub_no_recover: {:?}, sub_error_count: {:?} }}",
            self.old_parser,
            self.yylval,
            self.yychar,
            self.yyerrstatus,
